...
struct sprd_fchg_ops {
	...
	int (*get_adapt_svid)(struct sprd_fchg_info *info, u32 *svid);
	...
	int (*get_fchg_vol_now)(struct sprd_fchg_info *info, int *voltage_now);
	int (*enable_pps_mode)(struct sprd_fchg_info *info, bool enable);
	...
	int (*set_pd_fix_vol)(struct sprd_fchg_info *info, u32 input_vol);
