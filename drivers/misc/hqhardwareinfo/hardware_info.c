...
#define HARDWARE_INFO_LATTE "rmx3930"
#define HARDWARE_INFO_LATTE_A "rmx3933"
#define HARDWARE_INFO_LATTE_B "realme C63"
#define HARDWARE_INFO_LATTE_C "NARZO N63"
#define HARDWARE_INFO_LATTE_D "rmx3933"
...
int search_str(char* str, char **result)
{
    ...
    if((strlen(cmdline) < 1)||(strlen(pstr)<1)) {
        printk("search_str:pstr lens:%d,cmdline lens:%d", strlen(pstr) , strlen(cmdline));
        return -1;
    }
	...
	if((i >= strlen(cmdline) - 1)||(i <= 0)) {
        printk("search_str:i:%d,lens:%d", i , strlen(cmdline));
		return -1;
	}
...
static int hq_prj_name_setup()
{
	...
	if ((strncmp(prj_name, "24660", 5) == 0) || (strncmp(prj_name, "24661", 5) == 0) ||
		(strncmp(prj_name, "24662", 5) == 0)){
		prj_name_flag = 1;
	} else if ((strncmp(prj_name, "24663", 5) == 0) || (strncmp(prj_name, "24664", 5) == 0) ||
		(strncmp(prj_name, "24665", 5) == 0)){
		prj_name_flag = 2;
	} else if ((strncmp(prj_name, "24666", 5) == 0) || (strncmp(prj_name, "24667", 5) == 0) ||
		(strncmp(prj_name, "24668", 5) == 0)){
		prj_name_flag = 3;
	}else if (strncmp(prj_name, "24673", 5) == 0){
		prj_name_flag = 4;
	}else if (strncmp(prj_name, "24674", 5) == 0){
		prj_name_flag = 5;
	}
	return 0;
}
...
static int get_emmc_vendor_name(char* buff_name)
{
    ...
    pfile = filp_open_block(EMMC_VENDOR_NAME, O_RDONLY, 0);
    if (IS_ERR(pfile)) {
...
static unsigned long long get_emmc_version(void)
{
    ...
    pfile = filp_open_block(EMMC_VENDOR_VERSION, O_RDONLY, 0);
    if (IS_ERR(pfile)) {
...
static unsigned int get_emmc_size(void)
{
	...
	pfile = filp_open_block(emmc_file, O_RDONLY, 0);
	if (IS_ERR(pfile)) {
		printk("[HWINFO]: open emmc size file failed!\n");
		goto ERR_0;
	}
...
static ssize_t show_wifi(struct device *dev, struct device_attribute *attr, char *buf)
{
    if (prj_name_flag == 1) {
        ...
        return sprintf(buf, "wifi name :%s\n", HARDWARE_INFO_LATTE_A);
    } else if (prj_name_flag == 3) {
        return sprintf(buf, "wifi name :%s\n", HARDWARE_INFO_LATTE_B);
    } else if (prj_name_flag == 4) {
        return sprintf(buf, "wifi name :%s\n", HARDWARE_INFO_LATTE_C);
    } else if (prj_name_flag == 5){
        return sprintf(buf, "wifi name :%s\n", HARDWARE_INFO_LATTE_D);
    }else{
        return sprintf(buf, "wifi name :%s\n", "unknown");
    }
}
...
static ssize_t show_rear_als(struct device *dev, struct device_attribute *attr, char *buf)
{
	if (NULL != hwinfo_data.rear_als)
		return sprintf(buf, "rear_als name:%s\n", hwinfo_data.rear_als);
	else
		return sprintf(buf, "rear_als name:Not Found\n");
}
...
static DEVICE_ATTR(rear_als, 0644, show_rear_als, NULL);
...
static int HardwareInfo_driver_probe(struct platform_device *pdev)
{
...
#if 0
	cmd_file = filp_open_block(CMDLINE_PATH, O_RDONLY, 0);
	if (IS_ERR(cmd_file)) {
		error = PTR_ERR(cmd_file);
		printk("[HWINFO] filp_open_block failed! error:%d \n", error);
		return -ENOENT;
	} else {
		//fs = get_fs();
		//set_fs(KERNEL_DS);
    ...
    ret = of_property_read_string(cmdline_node, "bootargs", &name);
    if (!ret)
    {
        return_str = strstr(name, "lcd_id=");
        printk("read lcd_name: %s", return_str);
        strncpy(cmdline, return_str, strlen(return_str));
    }
#endif

	//printk("read cmdline %d:%s\n",(int)strlen(cmdline),cmdline);
