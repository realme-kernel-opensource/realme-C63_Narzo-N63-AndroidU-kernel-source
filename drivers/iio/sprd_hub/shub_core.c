...
/* likaisheng@BSP.SENSOR 2024/1/16, add for distinguish nfc device, start */
static int sensor_get_prj_name(char *buf, size_t len)
{
	...
	if (ret) {
		pr_err("%s: cannot read dt string\n", __func__);
		return ret;
	}
	...
	if (temp_name != NULL) {
		...
		strncpy(buf, temp_name, len);
	} else {
		pr_err("%s: fail to get temp_name\n", __func__);
		return -EINVAL;
	}
...
static int sensor_set_prj_name(struct shub_data *sensor)
{
	int err = 0;
	static char prj_name[6];

	err = sensor_get_prj_name(prj_name, sizeof(prj_name) - 1);
	if (err)
		return -1;
	printk("%s: prj_name=%s\n", __func__, prj_name);
	err = shub_send_command(sensor,
			HANDLE_MAX,
			SHUB_SET_PROJECT_NAME_SUBTYPE,
			prj_name,
			sizeof(prj_name));

	if (err < 0) {
		pr_err("%s: fail to set prj_name to cm4\n", __func__);
		return err;
	}
	return 0;
}
/* likaisheng@BSP.SENSOR 2024/1/16, add for distinguish nfc device, end */
...
static ssize_t sensor_init_flag_show(struct device *dev,
                struct device_attribute *attr, char *buf)
{
    int err;
    u32 number = 0;
...
static DEVICE_ATTR_RO(sensor_init_flag);
...
static ssize_t raw_data_rear_als_show(struct device *dev,
				 struct device_attribute *attr, char *buf)
{
	struct shub_data *sensor = dev_get_drvdata(dev);
	u8 data[2];
	...
	err = shub_sipc_read(sensor, SHUB_GET_REAR_ALS_RAWDATA_SUBTYPE, data,
			     sizeof(data));
	if (err < 0) {
		dev_err(&sensor->sensor_pdev->dev,
			"read RegMapR_GetRearAlsRawData failed!\n");
		return err;
	}
	return sprintf(buf, "%d\n", ptr[0]);
}
static DEVICE_ATTR_RO(raw_data_rear_als);
...
static void get_sensor_info(struct device *dev)
{
	...
	for (i = 0; i < ARRAY_SIZE(sensor->sensor_info_list); i++) {
		if (sensor->sensor_info_list[i].name[0] != 0) {
			...
			} else if (strstr(sensor->sensor_info_list[i].name, "rals_") != NULL) {
				get_hardware_info_data(HWID_REAR_ALS,sensor->sensor_info_list[i].name);
			}
