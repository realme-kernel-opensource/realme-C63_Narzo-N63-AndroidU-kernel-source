				      ...
				      const struct sprd_pd_message *msg)
{
	...
	case SPRD_PD_CTRL_VCONN_SWAP:
		...
		case SNK_READY:
			...
			if (port->tcpc->support_vconn && port->tcpc->support_vconn(port->tcpc)){
...
static int sprd_tcpm_pd_select_pdo(struct sprd_tcpm_port *port, int *sink_pdo, int *src_pdo)
{
	...
	for (i = 0; i < port->nr_source_caps; i++) {
		...
		default:
			...
			for (j = 0; j < port->nr_snk_default_pdo; j++) {
...
static int sprd_tcpm_pd_build_pps_request(struct sprd_tcpm_port *port, u32 *rdo)
{
	...
	case SPRD_PDO_TYPE_APDO:
		...
		op_ma = sprd_pdo_pps_apdo_max_current(pdo);
		sprd_tcpm_log(port, "max_mv = %d, max_ma = %d, out_mv = %d, op_ma = %d\n", max_mv, max_ma, out_mv, op_ma);
		break;
	default:
		sprd_tcpm_log(port, "Invalid PDO selected!");
		return -EINVAL;
...
{
	 if (port->send_discover && port->pd_capable) {
...
int sprd_tcpm_get_adapt_svid(struct sprd_tcpm_port *port, u32 *svid)
{
       int ret = 0;

       mutex_lock(&port->lock);
       *svid = SPRD_PD_IDH_VID(port->partner_ident.id_header);
       mutex_unlock(&port->lock);
       return ret;
}
EXPORT_SYMBOL_GPL(sprd_tcpm_get_adapt_svid);
...
static int sprd_tcpm_psy_get_pps_max_cur(struct sprd_tcpm_port *port,
					 union power_supply_propval *val)
{
	unsigned int k,max_src_mv,min_src_mv;
	unsigned int max_src_cur = 0;
	unsigned int src_ma = 0;
	u32 pdo;

	sprd_tcpm_log(port, "sprd_tcpm_psy_get_pps_max_cur val->intval %d",
					val->intval);
	for (k = 0; k < port->nr_source_caps; ++k) {
			pdo = port->source_caps[k];

			if(sprd_pdo_type(pdo) != SPRD_PDO_TYPE_APDO)
				continue;
			min_src_mv = sprd_pdo_pps_apdo_min_voltage(pdo);
			max_src_mv = sprd_pdo_pps_apdo_max_voltage(pdo);
			src_ma = sprd_pdo_pps_apdo_max_current(pdo);

			if(val->intval >= min_src_mv && val->intval <= max_src_mv && max_src_cur < src_ma) {
				sprd_tcpm_log(port, "sprd_tcpm_psy_get_pps_max_cur %d %d %d %d %d",
					min_src_mv, max_src_mv, k, src_ma, max_src_cur);
				max_src_cur = src_ma;
			}
	}

	return (int)max_src_cur;
}
					 ...
					 union power_supply_propval *val)
{
	if (port->pps_data.active) {
		...
		val->intval = sprd_tcpm_psy_get_pps_max_cur(port, val) * 1000;
	} else {
