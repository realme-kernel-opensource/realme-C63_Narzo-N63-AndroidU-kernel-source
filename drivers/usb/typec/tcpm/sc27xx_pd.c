...
#define SC27XX_TYPEC_NEED_VCONN_LDO    BIT(15)
...
static int sc27xx_pd_vconn_enable(struct sc27xx_pd *pd)
{
	int ret;

	if (!pd->vddldo) {
		sprd_pd_log(pd, "%s:get vddldo", __func__);
		pd->vddldo = devm_regulator_get_optional(pd->dev, "vddldo");
		if (IS_ERR(pd->vddldo)) {
			dev_err(pd->dev, "%s:failed to get vddldo supply\n", __func__);
			pd->vddldo = NULL;
			return -EINVAL;
		}
	}

	ret = regulator_enable(pd->vddldo);
	if (ret) {
		dev_err(pd->dev, "failed to enable ldo for typec vconn, ret = %d.\n", ret);
		return ret;
	}

	/* set vconn ldo enable */
	ret = regmap_update_bits(pd->regmap, pd->typec_base + SC27XX_TYPEC_PD_CFG,
                 SC27XX_TYPEC_VCCON_LDO_EN, SC27XX_TYPEC_VCCON_LDO_EN);
	if (ret) {
		dev_err(pd->dev, "failed to set ldo en, ret = %d.\n", ret);
		goto vconn_ldo_en_err;
	}

	/* vconn ldo is ready for typec controller switch vconn for analog */
	ret = regmap_update_bits(pd->regmap, pd->typec_base + SC27XX_TYPEC_PD_CFG,
                 SC27XX_TYPEC_VCCON_LDO_RDY, SC27XX_TYPEC_VCCON_LDO_RDY);
	if (ret) {
		dev_err(pd->dev, "failed to set ldo rdy, ret = %d.\n", ret);
		goto vconn_ldo_rdy_err;
	}

	ret = regmap_update_bits(pd->regmap, pd->typec_base + SC27XX_TYPEC_WC_REG,
                 SC27XX_TYPEC_PD_VCONN_SWAP, SC27XX_TYPEC_PD_VCONN_SWAP);
	if (ret) {
		dev_err(pd->dev, "failed to set wc, ret = %d.\n", ret);
		goto vconn_ldo_rdy_err;
	}

	sprd_pd_log(pd, "%s: enable vconn successfully", __func__);
	return 0;

vconn_ldo_rdy_err:
	sprd_pd_log(pd, "%s:vconn ldo rdy err", __func__);
	regmap_update_bits(pd->regmap, pd->typec_base + SC27XX_TYPEC_PD_CFG,
               SC27XX_TYPEC_VCCON_LDO_EN, 0);

vconn_ldo_en_err:
	sprd_pd_log(pd, "%s:vconn ldo en err", __func__);
	regulator_disable(pd->vddldo);

	return ret;
}

static int sc27xx_typec_vconn_disable(struct sc27xx_pd *pd)
{
	int ret;
	if (!pd->vddldo) {
		sprd_pd_log(pd, "%s:get vddldo", __func__);
	...
	ret = regulator_disable(pd->vddldo);
	if (ret) {
		dev_err(pd->dev, "failed to disable ldo for typec vconn, ret = %d.\n", ret);
		return ret;
	}
	...
	ret = regmap_update_bits(pd->regmap, pd->typec_base + SC27XX_TYPEC_PD_CFG,
                 SC27XX_TYPEC_VCCON_LDO_EN, 0);
	if (ret) {
		dev_err(pd->dev, "failed to disable ldo en, ret = %d.\n", ret);
		return ret;
	}

	/* vconn ldo is ready for typec controller switch vconn for analog */
	ret = regmap_update_bits(pd->regmap, pd->typec_base + SC27XX_TYPEC_PD_CFG,
                 SC27XX_TYPEC_VCCON_LDO_RDY, 0);
	if (ret) {
		dev_err(pd->dev, "failed to disable ldo rdy, ret = %d.\n", ret);
		return ret;
	}

	sprd_pd_log(pd, "%s: disable vconn successfully", __func__);
...
static int sc27xx_pd_set_vconn(struct tcpc_dev *tcpc, bool enable)
{
	...
	if (enable)
		ret = sc27xx_pd_vconn_enable(pd);
	else
		ret = sc27xx_typec_vconn_disable(pd);
...
static bool sc27xx_pd_support_vconn(struct tcpc_dev *tcpc)
{
    struct sc27xx_pd *pd = tcpc_to_sc27xx_pd(tcpc);
    bool support_vconn;
    ...
    if (pd->vddldo) {
        support_vconn = true;
        sprd_pd_log(pd, "support vconn");
    } else {
        support_vconn = false;
        sprd_pd_log(pd, "don't support vconn");
    }
    mutex_unlock(&pd->lock);

    return support_vconn;
}
...
static int sc27xx_pd_check_vbus_cc_status(struct sc27xx_pd *pd)
{
	...
	if (ret)
		return ret;
	sprd_pd_log(pd, "SC27XX_TYPEC_STATUS = 0x%x", val);
	if ((val & SC27XX_TYPEC_NEED_VCONN_LDO)
		|| ((val & SC27XX_TYPEC_PLUGIN_CABLE) == 4)) {
		pd->is_need_vconn = true;
		sprd_pd_log(pd, "need vconn");
    }
...
static void sc27xx_init_tcpc_dev(struct sc27xx_pd *pd)
{
	...
	pd->tcpc.support_vconn = sc27xx_pd_support_vconn;
}
