...
#define SW_AICL_POINT_4V5    4500
#define SW_AICL_POINT_4V535  4535
#define HW_AICL_POINT_4V5	 4500
#define HW_AICL_POINT_4V6	 4600
#define HW_AICL_POINT_4V7	 4700
#define BATT_VOL_4V35	4350
#define BATT_VOL_4V25   4250
#define BATT_VOL_4V14	4140
#define BATT_VOL_4V     4000
...
int oplus_sgm41512_get_adapter_type(void)
{
    return oplus_get_adapter_svid();
}
...
void sgm41512_set_aicl_point(int vbatt)
{
	...
	if (chip->vindpm_4V48) {
		if ((chip->hw_aicl_point != HW_AICL_POINT_4V7) && (vbatt > BATT_VOL_4V35)) {
			chip->hw_aicl_point = HW_AICL_POINT_4V7;
			chip->sw_aicl_point = SW_AICL_POINT_4V535;
		} else if (vbatt > BATT_VOL_4V14) {
			if ((chip->hw_aicl_point == HW_AICL_POINT_4V7) && (vbatt < BATT_VOL_4V25)) {
				chip->hw_aicl_point = HW_AICL_POINT_4V6;
			} else if (chip->hw_aicl_point == HW_AICL_POINT_4V5) {
				chip->hw_aicl_point = HW_AICL_POINT_4V6;
			}
			chip->sw_aicl_point = SW_AICL_POINT_4V535;
		} else if (chip->hw_aicl_point != HW_AICL_POINT_4V5) {
			chip->hw_aicl_point = HW_AICL_POINT_4V5;
			chip->sw_aicl_point = SW_AICL_POINT_4V5;
		}
		sgm41512_set_vindpm_vol(chip->hw_aicl_point);
	} else {
		if (chip->hw_aicl_point == HW_AICL_POINT_4V5 && vbatt > BATT_VOL_4V14) {
			chip->hw_aicl_point = HW_AICL_POINT_4V6;
			chip->sw_aicl_point = SW_AICL_POINT_4V535;
			sgm41512_set_vindpm_vol(chip->hw_aicl_point);
		} else if (chip->hw_aicl_point == HW_AICL_POINT_4V6 && vbatt < BATT_VOL_4V) {
			chip->hw_aicl_point = HW_AICL_POINT_4V5;
			chip->sw_aicl_point = SW_AICL_POINT_4V5;
			sgm41512_set_vindpm_vol(chip->hw_aicl_point);
		}
...
int sgm41512_unsuspend_charger(void)
{
	...
	if (g_oplus_chip) {
		if (g_oplus_chip->mmi_chg != 0  && oplus_pps_get_pps_mos_started() != true) {
		result = sgm41512_enable_charging();
		} else if(g_oplus_chip->mmi_chg == 0) {
		chg_err("set icl 100 when mmi_chg = %d\n",g_oplus_chip->mmi_chg);
		sgm41512_input_current_limit_without_aicl(100);
		}
	}
...
int sgm41512_hardware_init(void)
{
	...
	if (chip->vindpm_4V48) {
		chip->sw_aicl_point = SW_AICL_POINT_4V5;
		if (v_bat > BATT_VOL_4V35) {
			chip->hw_aicl_point = HW_AICL_POINT_4V7;
		} else if (v_bat > BATT_VOL_4V14) {
			chip->hw_aicl_point = HW_AICL_POINT_4V6;
		} else {
			chip->hw_aicl_point = HW_AICL_POINT_4V5;
		}
...
struct oplus_chg_operations  sgm41512_chg_ops = {
	...
	.oplus_chg_get_adapter_type = oplus_sgm41512_get_adapter_type,
...
static int sgm41512_parse_dt(struct chip_sgm41512 *chip)
{
	...
	chip->vindpm_4V48 = of_property_read_bool(chip->client->dev.of_node, "qcom,vindpm_4V48");
	chg_err("vindpm_4V48=%d\n", chip->vindpm_4V48);
	return ret;
}
...
static void sgm41512_bc12_retry_work(struct work_struct *work)
{
	...
	if (chip->is_sgm41512) {
		...
		if (sgm41512_get_iindet()) {
			...
			sgm41512_get_bc12(chip);
			//chg_err("delay 100ms to cancel_dpdm_high_impedance_state\n");
			//msleep(100);
			//sgm41512_request_dpdm(chip, false);
		} else {
...
static void sgm41512_start_bc12_retry(struct chip_sgm41512 *chip) {
	if (!chip)
		return;
	sgm41512_request_dpdm(chip, true);
	chg_err("delay 100ms to sgm41512_set_iindet!\n");
	msleep(100);
	sgm41512_set_iindet();
...
static void sgm41512_get_bc12(struct chip_sgm41512 *chip)
{
	...
	if (!chip->bc12_done) {
		...
		chg_err("vbus_stat=%d\n", vbus_stat);
		sgm41512_request_dpdm(chip, false);
		chg_err("cancel_dpdm_high_impedance_state before retry!\n", chip->bc12_retried);
		switch (vbus_stat) {
...
#define OPLUS_WAIT_RESUME_TIME	500
