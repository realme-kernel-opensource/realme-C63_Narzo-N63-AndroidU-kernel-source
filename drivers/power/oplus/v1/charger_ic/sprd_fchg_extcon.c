...
static int sprd_get_pps_voltage_now(struct sprd_fchg_info *info, u32 *now_vol)
{
	union power_supply_propval val;
	struct power_supply *psy_tcpm;
	int ret;
...
static int sprd_get_pps_current_max(struct sprd_fchg_info *info, int input_vol, u32 *max_cur)
{
	union power_supply_propval val;
	struct power_supply *psy_tcpm;
	int ret;
...
static int sprd_pd_fixed_adjust_voltage(struct sprd_fchg_info *info, u32 input_vol)
{
	...
	if (i == info->pd_source_cap.nr_source_caps && index == 0) {
		index = info->pd_source_cap.nr_source_caps -1;
	}
	...
	dev_err(info->dev, "%s, set pd input_vol %d, index = %d\n", __func__, input_vol, index);
...
static int sprd_get_pps_voltage_max(struct sprd_fchg_info *info, int input_vol, u32 *max_vol)
{
	return 0;
}
...
static int sprd_enable_pps_mode(struct sprd_fchg_info *info, bool enable)
{
	if (!info) {
		pr_err("%s[%d]: info is NULL pointer!!!\n", __func__, __LINE__);
		return -1;
	...
	return sprd_enable_pps(info, enable);
...
static int sprd_set_pd_fix_vol(struct sprd_fchg_info *info, u32 input_vol)
{
       int ret = 0;
...
static int sprd_pd_tcpm_get_adapt_svid(struct sprd_fchg_info *info, u32 *svid)
{
       struct sprd_tcpm_port *port;
       struct power_supply *psy_tcpm;
       int ret;
       ...
       ret = sprd_tcpm_get_adapt_svid(port, svid);
       if (ret) {
               dev_err(info->dev, "%s, failed to get adapt svid, ret = %d\n", __func__, ret);
               return ret;
       }
...
static int sprd_pd_get_adapt_svid(struct sprd_fchg_info *info, u32 *svid)
{
       struct sprd_tcpm_port *port;
       struct power_supply *psy_tcpm;
       int ret;
       ...
       ret = sprd_pd_tcpm_get_adapt_svid(info, svid);
       if (ret) {
               dev_err(info->dev, "%s, failed to get adapt svid, ret = %d\n", __func__, ret);
               return ret;
       }
...
static int sprd_get_adapt_svid(struct sprd_fchg_info *info, u32 *svid)
{
       int ret = 0;
       ...
       mutex_lock(&info->lock);
       dev_info(info->dev, "%s:%d, pd_extcon = %d\n", __func__, __LINE__, info->pd_extcon);
       ret = sprd_pd_get_adapt_svid(info, svid);
       if (ret)
               dev_err(info->dev, "%s, failed to get adapt svid, ret=%d\n", __func__, ret);
       mutex_unlock(&info->lock);
...
static int sprd_get_fchg_voltage_now(struct sprd_fchg_info *info, int *voltage_now)
{
	int ret = 0;
	...
	*voltage_now = 0;
	...
	mutex_lock(&info->lock);
	if (info->pps_enable && info->pps_active) {
		ret = sprd_get_pps_voltage_now(info, voltage_now);
		*voltage_now = *voltage_now / 1000;
		if (ret)
			dev_err(info->dev, "%s, failed to get pps vol now, ret=%d\n",
				__func__, ret);
	} else if (info->pps_enable || info->pd_enable) {
		ret = sprd_get_pps_voltage_now(info, voltage_now);
		*voltage_now = *voltage_now / 1000;
		if (ret)
			dev_err(info->dev, "%s, failed to get pd vol now, ret=%d\n",
				__func__, ret);
	} else {
			dev_err(info->dev, "%s, failed to get pps vol now, ret=%d\n",
				__func__, ret);
...
static int sprd_get_fchg_current_max(struct sprd_fchg_info *info, int input_vol, int *current_max)
{
	...
	*current_max = 0;
	...
	if (info->pps_enable && (!input_vol || info->pps_active)) {
		ret = sprd_get_pps_current_max(info, input_vol, current_max);
		if (ret)
			dev_err(info->dev, "%s, failed to get pps current max, ret=%d\n",
				__func__, ret);
	} else if (info->pps_enable || info->pd_enable) {
