...
static int sc8547_parse_dt(struct oplus_voocphy_manager *chip)
{
	...
	/*Only SGM41512+SC8547B: OVPGATE remains closed when otg en is set after surge testing. Cancel the otg en setting when the otg device is not used*/
	chip->remove_otg_en = of_property_read_bool(node, "qcom,remove_otg_en");
	chg_err("remove_otg_en %d\n", chip->remove_otg_en);
...
void voocphy_set_otg_en(bool status)
{
	if(!oplus_voocphy_mg ||!oplus_voocphy_mg->client||!oplus_voocphy_mg->remove_otg_en)
		return;

	if (status) {
		sc8547_write_byte(oplus_voocphy_mg->client, SC8547A_ADDR_OTG_EN, 0x04);
		pr_err("sc8547_set_otg_en done");
	} else{
		sc8547_write_byte(oplus_voocphy_mg->client, SC8547A_ADDR_OTG_EN, 0x00);
		pr_err("sc8547_reset_otg_en done");
	}
}
EXPORT_SYMBOL(voocphy_set_otg_en);
