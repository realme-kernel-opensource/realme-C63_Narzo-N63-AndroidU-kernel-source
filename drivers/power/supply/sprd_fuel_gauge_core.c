...
static int sprd_fgu_get_capacity(struct sprd_fgu_data *data, int *cap)
{
...
#else
	int ret, delta_cap, temp_cap, total_mah, avg_temp;
#endif
	struct sprd_fgu_info *fgu_info = data->fgu_info;
	...
	*cap = delta_cap + data->init_cap;
	...
	delta_cap = DIV_ROUND_CLOSEST(data->cc_uah, total_mah);
	*cap = delta_cap + data->init_cap;
	data->normal_temp_cap = *cap;
	...
	dev_info(data->dev, "init_cap = %d, normal_cap = %d, "
		 ...
		 fgu_info->vol_1000mv_adc, fgu_info->cur_1000ma_adc, data->cc_uah / 1000);
	if (*cap < 0) {
		*cap = 0;
		dev_err(data->dev, "ERORR: normal_cap is < 0, adjust!!!\n");
		sprd_fgu_dump_info(data);
				 ...
				 union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		if (val->intval == CM_BOOT_CAPACITY) {
			...
			ret = fgu_info->ops->read_last_cap(fgu_info, &value);
			if (ret) {
				goto error;
			}
...
static int __init sprd_fgu_driver_init(void)
{


    pr_err("%s:num =%d\n",__func__, __LINE__);

	return platform_driver_register(&sprd_fgu_driver);
}

static void __exit sprd_fgu_driver_exit(void)
{
	platform_driver_unregister(&sprd_fgu_driver);
}
subsys_initcall(sprd_fgu_driver_init);
module_exit(sprd_fgu_driver_exit);
