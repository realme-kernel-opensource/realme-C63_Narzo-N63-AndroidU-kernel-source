...
static irqreturn_t nfc_clk_irq_thread(int irq, void *dev_id)
{
    struct nfc_dev *info = dev_id;
    bool value;
    struct platform_gpio *nfc_gpio = &info->configs.gpio;

    value = gpio_get_value(nfc_gpio->clk_irq) > 0 ? true : false;
    //pr_err("%s: enter,CLK state %d,value %d\n", __func__ ,info->clk_state,value);
    if (value == info->clk_state)
        return IRQ_HANDLED;

    //Fix gnss bug 7196819
    //open nfc clk when NFC on
    //if (value) {
    //        pr_err("%s: nfc_clk_irq_thread on skip\n", __func__);
    //} else {
    //        pr_err("%s: nfc_clk_irq_thread off skip\n", __func__);
    //}
...
void nfc_clk_ctl_enable(struct nfc_dev *info)
{
    int ret;
    pr_err("%s: enter\n", __func__);
    if(info == NULL){
        pr_err("%s: nfc info is null\n", __func__);
        return;
    }
    if (info->clk_ctl){
        pr_err("%s: clk_ctl enabled already\n", __func__);
        return;
    }

    info->clk_state = false;
    ret = request_threaded_irq(info->clk_irq, NULL, nfc_clk_irq_thread,
                                IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING | IRQF_ONESHOT,
                                NFC_I2C_DEV_ID, info);
    if (ret < 0)
    pr_err("%s: failed to register CLK REQ IRQ handler\n", __func__);

    info->clk_ctl = true;
}

void nfc_clk_ctl_disable(struct nfc_dev *info)
{
    pr_err("%s: enter\n", __func__);
    if(info == NULL){
        pr_err("%s: nfc info is null\n", __func__);
    ...
    if (!info->clk_ctl){
        pr_err("%s: clk_ctl disabled already\n", __func__);
        return;
    }
    free_irq(info->clk_irq, info);
    if (info->clk_state) {
        pr_err("%s: clk_ctl skip\n", __func__);
...
int nfc_i2c_dev_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	nfc_dev->nfc_clk_irq_enable = nfc_clk_ctl_enable;
	nfc_dev->nfc_clk_irq_disable = nfc_clk_ctl_disable;
	...
	client->irq = ret;
        ret = configure_gpio(nfc_gpio->clk_irq, GPIO_IRQ);
        if (ret <= 0) {
                pr_err("%s: unable to request nfc clk_irq gpio [%d]\n", __func__,
                       nfc_gpio->clk_irq);
                goto err_free_gpio;
        }
        nfc_dev->clk_irq = ret;
