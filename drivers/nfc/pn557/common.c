		 ...
		 uint8_t interface)
{
	...
	if (interface == PLATFORM_IF_I2C) {
        ...
        nfc_gpio->clk_irq = of_get_named_gpio(np, DTS_CLK_GPIO_STR, 0);
        if ((!gpio_is_valid(nfc_gpio->clk_irq))) {
                pr_err("%s: clk irq gpio invalid %d\n", __func__, nfc_gpio->clk_irq);
                return nfc_gpio->clk_irq;
        }
	...
	pr_info("%s: %d, %d, %d, %d\n", __func__, nfc_gpio->irq, nfc_gpio->ven,
		nfc_gpio->dwl_req,nfc_gpio->clk_irq);
	return 0;
}
